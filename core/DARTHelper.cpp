...
MakeBodyNode(const SkeletonPtr& skeleton,BodyNode* parent,Joint::Properties* joint_properties,const std::string& joint_type,dart::dynamics::Inertia inertia)
{
    ...
    else if(joint_type == "Ball")
    {
        ...
        shapeNode->setRelativeTransform(prop->mT_ChildBodyToJoint);
...
BuildFromFile(const std::string& path,bool create_obj)
{
    ...
    for(TiXmlElement* node = skeleton_elem->FirstChildElement("Node");node != nullptr;node = node->NextSiblingElement("Node"))
    {
        ...
        Eigen::Vector3d obj_offset = Eigen::Vector3d::Identity();
        if(body->Attribute("obj"))
        {
            obj_file = body->Attribute("obj");
            if(body->Attribute("obj_offset")){
                obj_offset = string_to_vector3d(body->Attribute("obj_offset"));
            }
        }
        ...
        if(obj_file!="None" && create_obj)
        {
            ...
            MeshShapePtr visual_shape = std::shared_ptr<MeshShape>(new MeshShape(Eigen::Vector3d(1,1,1),scene));
...
Muscles* MASS::BuildMusclesFromFile(const std::string& path,SkeletonPtr skeleton /* to attach to */){
    TiXmlDocument doc;
    ...
    for(TiXmlElement* unit = muscledoc->FirstChildElement("Unit");unit!=nullptr;unit = unit->NextSiblingElement("Unit"))
    {
        ...
        Muscle* muscle = new Muscle(name, f0, lm, lt, pa, lmax);
        ...
        for(TiXmlElement* waypoint = unit->FirstChildElement("Waypoint");waypoint!=nullptr;waypoint = waypoint->NextSiblingElement("Waypoint"))
        {
            ...
            if(i==0||i==num_waypoints-1)
                // if(true)
                muscle->AddAnchor(skeleton->getBodyNode(body), glob_pos);
            else
                muscle->AddAnchor(skeleton, skeleton->getBodyNode(body), glob_pos, 2);

            i++;
        }
        muscles->addMuscle(muscle);
    }
    return muscles;
}